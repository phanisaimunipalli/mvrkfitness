...
export default class Analytics extends Component {
  ...
  handleWeekChange1 = (amount) => {
    const { selectedWeek } = this.state
    const nextWeek = selectedWeek.clone().add(amount, 'week')

    // Prevent selecting future weeks
    if (nextWeek.isAfter(moment())) {
      return
    }

    const nextWeekData = this.fetchDataForWeek(nextWeek)
    this.setState({
  ...
  componentDidMount() {
    ...
    this.getChartData()
    const { selectedWeek } = this.state
    const currentWeekData = this.fetchDataForWeek(selectedWeek)
  ...
  getData = (selectedData) => {
    ...
    const data = [
      { date: weekStart.clone().add(0, 'day'), classes: 15, enrollments: 20 },
      { date: weekStart.clone().add(1, 'day'), classes: 4, enrollments: 10 },
  ...
  fetchDataForCurrentWeek = (week) => {
    // Assuming Mydata is an array of objects with a 'date' property
    const dataForCurrentWeek = this.Mydata.filter((item) => {
      ...
      return (
        itemDate.isSameOrAfter(startOfWeek, 'day') &&
        itemDate.isSameOrBefore(startOfWeek, 'week')
      )
    })
    console.log(dataForCurrentWeek)
    return dataForCurrentWeek
  }

  fetchDataForWeek = (week) => {
    const previousWeek1 = week.clone().subtract(1, 'week')
    console.log('previousWeek1', previousWeek1)
    const previousWeek2 = week.clone().subtract(2, 'week')
    // alert(previousWeek2)
    const dataForWeek = [] // Placeholder for the data

    // Fetch data for the specified week
    const fetchDataForCurrentWeek = this.fetchDataForCurrentWeek(week)
    dataForWeek.push(...fetchDataForCurrentWeek)

    // Fetch data for previousWeek1
    const fetchDataForPreviousWeek1 = this.fetchDataForCurrentWeek(
      previousWeek1,
    )
    dataForWeek.push(...fetchDataForPreviousWeek1)

    // Fetch data for previousWeek2
    const fetchDataForPreviousWeek2 = this.fetchDataForCurrentWeek(
      previousWeek2,
    )
    dataForWeek.push(...fetchDataForPreviousWeek2)

    // Return the data for the specified week and the previous two weeks
    return dataForWeek
  }
  ...
  render() {
    ...
    const data = [
      { date: weekStart.clone().add(0, 'day'), classes: 5, enrollments: 20 },
      { date: weekStart.clone().add(1, 'day'), classes: 4, enrollments: 30 },
      { date: weekStart.clone().add(2, 'day'), classes: 6, enrollments: 5 },
      { date: weekStart.clone().add(3, 'day'), classes: 3, enrollments: 20 },
      { date: weekStart.clone().add(4, 'day'), classes: 7, enrollments: 10 },
      { date: weekStart.clone().add(5, 'day'), classes: 2, enrollments: 20 },
      { date: weekStart.clone().add(6, 'day'), classes: 1, enrollments: 12 },
    ]
    ...
    /*     const filteredData4 = Mydata.filter((item) => {
      const itemDate = moment(item.date) // Assuming each data item has a "date" property
      return (
        itemDate.isSameOrBefore(selectedWeek.endOf('week')) &&
        itemDate.isSameOrAfter(selectedWeek.startOf('week'))
      )
    })
    ...
    return (
      <div class="chart-container">
        ...
        <div class="chart">
          ...
          /> */}
            ...
            <Bar
              ...
              background={{ fill: '#eee' }}
            />
            <Bar dataKey="enrollments" fill="lightblue" />
          </BarChart>
        </div>
        ...
        <div class="chart">
          <h1>Gym Hours</h1>
          <div>
            <button onClick={() => this.handleWeekChange1(-1)}>
              Prev Week
            </button>
            <DatePicker
              selected={selectedWeek.toDate()}
              ...
              dateFormat="MM/dd/yyyy"
            />
            <button onClick={() => this.handleWeekChange1(1)}>Next Week</button>
          </div>
        ...
        <div class="chart">
          ...
          >
            ...
            <Tooltip />
            <Legend />
            <Bar dataKey="visitors" fill="lightblue" />
            {/*  <Bar dataKey="" fill="#f5f5f5" />
            <Bar dataKey="" fill="#f5f5f5" /> */}
            {/*    <Bar dataKey="Weekday" fill="#82ca9d" />
