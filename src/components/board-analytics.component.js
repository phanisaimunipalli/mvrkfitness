...
export default class Analytics extends Component {
  ...
  getData = (selectedData) => {
    ...
    const data = [
      { date: weekStart.clone().add(0, 'day'), classes: 15, enrollments: 10 },
      { date: weekStart.clone().add(1, 'day'), classes: 4, enrollments: 5 },
  ...
  getFilteredData = () => {
    const { selectedRange, data } = this.state
    const currentDate = new Date()

    return data.filter((entry) => {
      const entryDate = new Date(entry.name)
      return entryDate <= currentDate && entryDate >= selectedRange
    })
  }
  ...
  render() {
    ...
    const filteredData3 = this.getFilteredData()
    ...
    const data = [
      { date: weekStart.clone().add(0, 'day'), classes: 5, enrollments: 10 },
      { date: weekStart.clone().add(1, 'day'), classes: 4, enrollments: 20 },
      { date: weekStart.clone().add(2, 'day'), classes: 6, enrollments: 35 },
      { date: weekStart.clone().add(3, 'day'), classes: 3, enrollments: 15 },
      { date: weekStart.clone().add(4, 'day'), classes: 7, enrollments: 40 },
      { date: weekStart.clone().add(5, 'day'), classes: 2, enrollments: 10 },
      { date: weekStart.clone().add(6, 'day'), classes: 1, enrollments: 5 },
    ...
    return (
      <div class="chart-container">
        ...
        <div class="chart">
          <h1>Gym Hours</h1>
          <div>
            <button onClick={() => this.handleRangeChange(-1)}>
              Prev Week
            </button>
            <DatePicker
              selected={selectedWeek.toDate()}
              ...
              dateFormat="MM/dd/yyyy"
            />
            <button onClick={() => this.handleRangeChange(1)}>Next Week</button>
          </div>
          <MyLineChart
            width={500}
            height={300}
